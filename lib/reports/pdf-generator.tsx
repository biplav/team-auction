...
import { AuctionSummaryPDF } from "./templates/auction-summary-pdf";
import type {
  AuctionSummaryData,
  TeamDetailedData,
  PlayerAuctionData,
 ...
 */
export async function generateAuctionSummaryPDF(
  data: AuctionSummaryData
): Promise<Buffer> {
  const pdfBuffer = await renderToBuffer(<AuctionSummaryPDF data={data} />);
  return pdfBuffer;
}

/**
 * Generate Team Detailed PDF
 * (Simplified version - can be expanded with custom template)
 */
export async function generateTeamDetailedPDF(
  data: TeamDetailedData
): Promise<Buffer> {
  // For now, use a simple approach - in production you'd create a dedicated template
  ...
  const doc = (
    <Document>
      <Page size="A4" style={styles.page}>
        <Text style={styles.title}>Team Report: {data.team.name}</Text>
        <View style={styles.section}>
          <Text style={styles.label}>Owner</Text>
          <Text style={styles.value}>{data.team.ownerName || "N/A"}</Text>
        </View>
        <View style={styles.section}>
          <Text style={styles.label}>Players</Text>
          <Text style={styles.value}>{data.statistics.playerCount}</Text>
        </View>
        <View style={styles.section}>
          <Text style={styles.label}>Total Spent</Text>
          <Text style={styles.value}>
            ₹{(data.statistics.totalSpent / 10000000).toFixed(2)}Cr
          </Text>
        </View>
      </Page>
    </Document>
...
/**
 * Generate Player Auction PDF
 */
export async function generatePlayerAuctionPDF(
  ...
  const styles = StyleSheet.create({
    ...
    title: { fontSize: 24, marginBottom: 20 },
    section: { marginBottom: 15 },
    playerRow: { flexDirection: "row", marginBottom: 5, fontSize: 10 },
  });

  const doc = (
    <Document>
      <Page size="A4" style={styles.page}>
        <Text style={styles.title}>Player Auction Report</Text>
        <Text style={styles.section}>Auction: {data.auction.name}</Text>
        <Text style={styles.section}>Sold Players: {data.soldPlayers.length}</Text>
        <Text style={styles.section}>Unsold Players: {data.unsoldPlayers.length}</Text>
      </Page>
    </Document>
  );
...
/**
 * Generate Bidding History PDF
 */
export async function generateBiddingHistoryPDF(
  ...
  const doc = (
    <Document>
      <Page size="A4" style={styles.page}>
        <Text style={styles.title}>Bidding History Report</Text>
        <Text style={styles.section}>Auction: {data.auction.name}</Text>
        <Text style={styles.section}>Total Bids: {data.allBids.length}</Text>
      </Page>
    </Document>
  );
...
/**
 * Generate Budget Analysis PDF
 */
export async function generateBudgetAnalysisPDF(
  ...
  const doc = (
    <Document>
      <Page size="A4" style={styles.page}>
        <Text style={styles.title}>Budget Analysis Report</Text>
        <Text style={styles.section}>Auction: {data.auction.name}</Text>
        <Text style={styles.section}>Teams Analyzed: {data.teams.length}</Text>
      </Page>
    </Document>
  );
